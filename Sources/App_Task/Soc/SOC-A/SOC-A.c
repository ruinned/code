...
#define SOC_ONE_HOUR     3600U // unit:1s
...
GeneralReturnType SOC_EstmatOCV(void)
{
    ...
    uWord wtmp=1;
    uWord soh;
    uWord volt, soc_tmp = 0;
    uLWord llwd = 0;
    uLWord standby_time = 0;
    uLWord bat_rated_ah;
    uLWord bat_actual_ah, bat_actual_ah_bak;
    const sSOC_SocToVoltType  *SocOcvPtr;
    const sSOC_SocToVoltType  *SocStepUpPtr;
    const sSOC_SocToVoltType  *SocStepDownPtr;
    const sSOC_TemperToAhType *SocTempPtr;
     
    SocOcvPtr = &SocToOcvArray[0][0];
    SocStepUpPtr = &SocToOcvArrayStepUp[0][0];
    SocStepDownPtr = &SocToOcvArrayStepDown[0][0];
    ...
    err_code = BmsSys_CalcStandbyTime(&standby_time);

    if ((SOC_EstmatDataIn.MinV > 0) && (SOC_EstmatDataIn.MinV <= 4500)
            && (SOC_ConfigPara.Ocv_Standby_Time <= standby_time))    
    {
        // delta voltage >= 100mv
        if((SOC_EstmatDataIn.AvgV - SOC_EstmatDataIn.MinV) >= SOC_ConfigPara.Delta_Min_V)
        {
            volt = SOC_EstmatDataIn.AvgV;
            err_code = (GeneralReturnType)SOC_ERRCODE_AVGV_USED;
        }
        else
        {
            volt = SOC_EstmatDataIn.MinV;
        }
        
        if(SOC_120_HOUR <= standby_time)
        {
...
//#ifdef OCV_INIT_ESTMAT_EN
        if(SOC_FUN_ENABLE == SOC_MacroSwitch.Ocv_Init_Estmat)
        {
            ...
            if( ((volt >= SOC_ConfigPara.Ocv_Linear_Range_L1) && (volt <= SOC_ConfigPara.Ocv_Linear_Range_H1)) 
             || ((volt >= SOC_ConfigPara.Ocv_Linear_Range_L2) && (volt <= SOC_ConfigPara.Ocv_Linear_Range_H2)) )
            {
                if(volt <= (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point) ) -> Volt)
                {
                   soc_tmp = (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point))->Soc;
                   
                }
                else if(volt >= (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point) + (SOC_Macro.Soc_Max_Soc_Point - 1))->Volt)
                {
                ...
                else
                {
                    ...
                    {
                        if(volt == (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point) + i)->Volt)
                        {
                        ...
                        else
                        {
                            if(volt < (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point) + i)->Volt)
                            {
                                ...
                                soc_tmp = (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point) + i - 1)->Soc
                                          + (uWord)((uLWord)(volt - (SocOcvPtr + (t * SOC_Macro.Soc_Max_Soc_Point) + i - 1)->Volt) *100 / llwd);
                                break ;
                            }
                        }
                    }
                ...
                /* calc battery capacity by min temper */
                if(SOC_FUN_ENABLE == SOC_MacroSwitch.ActualAh_Temper_Estmat)
                {
                    ...
                    llwd = bat_rated_ah * (1000 - soc_tmp) / 1000;
                    ...
                    err_code = EE_SaveVar(BOTHEE, SYSTEMST_RATED_ACTUAL_CAPACITY);
                }
...
//#ifdef OCV_STEP_ESTMAT_EN
        if(SOC_FUN_ENABLE == SOC_MacroSwitch.Ocv_Step_Estmat)
        {
            SOC_EstmatDataCalc.FlagSocToOcvArrayUsed = SOC_FLAG_OK;
            SOC_EstmatDataCalc.FlagDynamicEstmatEnable = SOC_FLAG_ENABLE;

            if((volt >= SOC_ConfigPara.Ocv_Step_Range_L) && (volt <= SOC_ConfigPara.Ocv_Step_Range_H))
            {
                // feature point estimate
                soc_tmp = 0;
                for(i = 0; i < 6; i++)// not less than
                {
                    if((volt > (SocStepUpPtr + (t * 6) + i)->Volt)
                            && (SOC_AhIntgl.Soc < (SocStepUpPtr + (t * 6) + i)->Soc))
                    {
                        soc_tmp = (SocStepUpPtr + (t * 6) + i)->Soc;
                        break;
                    }
                }

                for(i = 0; i < 4; i++)// not more than
                {
                    if((volt < (SocStepDownPtr + (t * 4) + i)->Volt)
                            && (SOC_AhIntgl.Soc > (SocStepDownPtr + (t * 4) + i)->Soc))
                    {
                        soc_tmp = (SocStepDownPtr + (t * 4) + i)->Soc;
                        break;
                    }
                }
                ...
                if(soc_tmp != 0)
                {
                    ...
                    SOC_EstmatDataOut.TargetValue = soc_tmp;
                    SOC_EstmatDataOut.EstmatType = ESTMAT_TYPE_OCV_STEP;
                }
            }
        }
//#endif// #ifdef OCV_STEP_ESTMAT_EN
...
GeneralReturnType SOC_EstmatCharge(void)
{
    GeneralReturnType err_code = ERR_OK;
    uByte i, c, t;
    uWord soc_tmp, min_v, max_v, chg_ratio = 0;
    
    const sSOC_SocToVoltType *SocChgPreHtPtr;
    const sSOC_SocToVoltType *SocChgPreLtPtr;
   
    SocChgPreHtPtr = &SocToVoltArrayChgPre_HT[0][0];
    SocChgPreLtPtr = &SocToVoltArrayChgPre_LT[0][0];  
    
//#ifdef CHG_PRE_ESTMAT_EN
    if(SOC_FUN_ENABLE == SOC_MacroSwitch.Chg_Pre_Estmat)
    {
        // pre charge estimate
        if( (SOC_EstmatDataCalc.CntSysStart >= WCNT_10S)
         && (CHG_ST_ON == Chg_GetChgSt()) )// charging
        {
            if(OFFSET_CUR > SOC_EstmatDataIn.AvgCur)
            {
                chg_ratio = (uWord)((uLWord)(OFFSET_CUR - SOC_EstmatDataIn.AvgCur) *100
                                    / (SOC_AhIntgl.MaxAh /100));// 0.1A 0.001Ah and *100;
            }

            if(chg_ratio <= 24 && chg_ratio > 3)// 0.2C
            {
                c = 0;
            }
            else if(chg_ratio <= 38 && chg_ratio > 24)// 0.3C
            {
                c = 1;
            }
            else if(chg_ratio <= 70 && chg_ratio > 38)// 0.5C
            {
                c = 2;
            }
            else if(chg_ratio <= 120 && chg_ratio > 70)// 1C
            {
                c = 3;
            }
            else if(chg_ratio <= 3)
            {
                c = 4;
                err_code = (GeneralReturnType)SOC_ERRCODE_CUR_LOW;
            }
            else
            {
                c = 4;
                err_code = (GeneralReturnType)SOC_ERRCODE_CUR_HIGH;
            }
            
            if(SOC_EstmatDataIn.MinT >= 55)
            {
                t = 1;
            }
            else
            ...
            if(c <= 3)
            {
                max_v = SOC_EstmatDataIn.MaxV;

                if(1 == t)
                {
                    for(i = 0; i < 2; i++)// not less than
                    {
                        if((max_v > (SocChgPreHtPtr + c * 4 + i)->Volt)
                                && (SOC_AhIntgl.Soc < (SocChgPreHtPtr + c * 4 + i)->Soc))
                        {
                            soc_tmp = (SocChgPreHtPtr + c * 4 + i)->Soc;
                            break;
                        }
                    }
                }
                else
                {
                    for(i = 0; i < 2; i++)// not less than
                    {
                        if((max_v > (SocChgPreLtPtr + c * 4 + i)->Volt)
                                && (SOC_AhIntgl.Soc < (SocChgPreLtPtr + c * 4 + i)->Soc))
                        {
                            soc_tmp = (SocChgPreLtPtr + c * 4 + i)->Soc;
                        break;
                        }
                    }
                }
                ...
                if((SOC_EstmatDataIn.AvgV - SOC_EstmatDataIn.MinV) >= SOC_ConfigPara.Delta_Min_V)
                {
                    min_v = SOC_EstmatDataIn.AvgV;
                    err_code = (GeneralReturnType)SOC_ERRCODE_AVGV_USED;
                }
                else
                {
                    min_v = SOC_EstmatDataIn.MinV;
                }

                if(1 == t)
                {
                    for(i = 2; i < 4; i++)// not less than
                    {
                        if((min_v > (SocChgPreHtPtr + c * 4 + i)->Volt)
                                && (SOC_AhIntgl.Soc < (SocChgPreHtPtr + c * 4 + i)->Soc))
                        {
                            soc_tmp = (SocChgPreHtPtr + c * 4 + i)->Soc;
                            break;
                ...
                else
                {
                    for(i = 2; i < 4; i++)// not less than
                    {
                        if((min_v > (SocChgPreLtPtr + c * 4 + i)->Volt)
                                && (SOC_AhIntgl.Soc < (SocChgPreLtPtr + c * 4 + i)->Soc))
                        {
                            soc_tmp = (SocChgPreLtPtr + c * 4 + i)->Soc;
                        break;
            ...
            if((soc_tmp != 0) && (SOC_EstmatDataOut.EstmatType < ESTMAT_TYPE_CHG_PRE))
            {
                SOC_EstmatDataOut.FlagValid = SOC_FLAG_OK;
                ...
                SOC_EstmatDataOut.TargetValue = soc_tmp;
                SOC_EstmatDataOut.EstmatType = ESTMAT_TYPE_CHG_PRE;
            }
        }
    }
//#endif// #ifdef CHG_PRE_ESTMAT_EN

//#ifdef CHG_END_ESTMAT_EN
    if(SOC_FUN_ENABLE == SOC_MacroSwitch.Chg_End_Estmat) 
    {
        // charging and ready to end
        if((CHG_STAGE_READY_TO_END == Chg_GetChgStage()) && (CHG_ST_ON == Chg_GetChgSt()))
        {
            if((SOC_AhIntgl.Soc < 1000) && (SOC_EstmatDataOut.EstmatType < ESTMAT_TYPE_CHG_END))
            {
                if(SOC_EstmatDataIn.AvgV >= SOC_ConfigPara.AvgV_Chg_End)
                {
                    SOC_EstmatDataOut.FlagValid = SOC_FLAG_OK;
                    SOC_EstmatDataOut.DeltaValue = 1000 - SOC_AhIntgl.Soc;
                    SOC_EstmatDataOut.FlagDir = SOC_DIR_PLUS;
                    SOC_EstmatDataOut.TargetValue = 1000;
                    SOC_EstmatDataOut.EstmatType = ESTMAT_TYPE_CHG_END;
                }
                else
                {
                    err_code = (GeneralReturnType)SOC_ERRCODE_AVGV_LOW;
                }
            }
        }
    }
//#endif// #ifdef CHG_END_ESTMAT_EN
...
GeneralReturnType SOC_EstmatProcess(void)
{
    ...
    if( (SOC_FLAG_FAIL == SOC_EstmatDataCalc.FlagOcvEstmatFinish)
     ...
     && (SOC_EstmatDataIn.MinT > 0)
     && (SOC_EstmatDataIn.MinT < 0xf0)
    ...
    if( (SOC_EstmatDataIn.MinT > 0)
     && (SOC_EstmatDataIn.MinT < 0xf0)
     && (SOC_EstmatDataCalc.CntSysStart > WCNT_600MS) 
     && (SOC_EstmatDataCalc.CntSysStart < WCNT_5S) 
     && (SOC_FLAG_FAIL == SOC_EstmatDataCalc.FlagTemperEstmatFinish) )
    {
        SOC_EstmatActualAH();
    }

    err_code = SOC_EstmatCharge();
    SOC_EstmatDchDyn();
    SOC_EstmatLimit();
...
//#ifdef SOC_SMOOTH_EN
         ...
         && (SOC_FLAG_DISABLE == SOC_EstmatDataOut.FlagOcvOnce) )
        {
             ...
             && (SOC_EstmatDataOut.TargetValue <= 300))
            {
             ...
             && (SOC_EstmatDataOut.TargetValue <= 300))
            {
                // speed up smooth for ESTMAT_TYPE_OCV_INIT
                SOC_Smooth.TimeInterval = WCNT_2S;// 0.1%/2S equal to 0.5%/10S and 3%/1min
            }
            ...
            else if(ESTMAT_TYPE_CHG_END == SOC_EstmatDataOut.EstmatType)
            {
                SOC_Smooth.TimeInterval = WCNT_100MS;
            }
